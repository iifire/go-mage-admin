...
mro.formDialog = (function(){
	...
	function _openDialog(url,width,callbacks,option){
		$.ajax({
			...
			if($('.mro-wrapper').find('.mio-dg-wrap').length>0){//如果是已经打开过窗口
				 _zId = _zId+$('.mro-wrapper').find('.mio-dg-wrap').length;
			}
			var _dgwIndex    = 'mio-dg-wrap'+_zId;//弹窗id
			var _dgsIndex    = 'mio-dg-shade'+_zId;//遮罩id
			...
			var _rizeAndGap  = 'right:10px;width:'+_width+'px !important;'+'margin-left:'+(1280-_width)+'px !important;';
			//弹窗content样式
				...
				if(_option.btns.btnArr.length>0){
					for(var i = 0;i < _option.btns.btnArr.length;i++){
						_btnHtml += /*'<div class="dg-btn-wrap" style="'+_btnwrapStyl+'">'*/
								 '<a href="javascript:;" '+(_option.btns.btnArr[i].action? ' data-action="'+_option.btns.btnArr[i].action+'" ':'')+' class="'+_option.btns.btnClass+' '+_option.btns.btnArr[i].btnColor+' '+_option.btns.btnArr[i].classClick+'">'+_option.btns.btnArr[i].txt+'</a>';
								/*'</div>';*/
					}
				}
			}
			...
			//弹窗初始化
			var _$dgwrap = $('<div class="mio-dg-wrap" style="'+_rizeAndGap+'" id="'+_dgwIndex+'" data-index="'+_zId+'">' +
							...
							'</div>'+
							'</div>');//新建窗体wrap
			var _$dgshade = $('<div class="mio-dg-shade" id="'+_dgsIndex+'"></div>');//窗体的遮罩
			$('.mro-wrapper').append(_$dgshade,_$dgwrap);//弹窗窗体
			//窗口遮罩向下展开(默认300毫秒)
			...
			_$dgwrap.slideDown(300,function(){
				...
				//鼠标移入弹窗中，禁止外面的滚动条
				_scollbar.hover(function () {
					mro.scrollEvent.disableScroll();
				}, function () {
					mro.scrollEvent.enableScroll();
				});
	...
	function _closeCurrentDg(index){
		var _currentContent = $('#mio-dg-wrap'+index/*$(content).attr('id')*/);//当前弹窗层
		var _currentShade = $('#mio-dg-shade'+index/*$(content).attr('data-index')*/);//当前遮罩层
	...
	function _close(index){
		var _currentContent = $('#mio-dg-wrap'+index);//当前弹窗层
		var _currentShade = $('#mio-dg-shade'+index);//当前遮罩层
		_currentContent.slideUp(300,function(){
			_currentContent.remove();
			_currentShade.remove();
			mro.scrollEvent.enableScroll();
	...
	function _closeAllDg(){
		$('.mio-dg-wrap').slideUp(300,function(){
			$('.mio-dg-wrap').remove();
			$('.mio-dg-shade').remove();
			mro.scrollEvent.enableScroll();
		});
		//$('.mio-dg-shade').slideUp(300,function(){
		//	$('.mio-dg-shade').remove();
		//});
	}
...
mro.scrollEvent = (function () {
    ...
    function _enableScroll() {
        if (document.addEventListener) {
            document.addEventListener('mousewheel', enAbleScroll, false); //IE9, Chrome, Safari, Opera
            document.addEventListener('wheel', enAbleScroll, false); //Firefox
            document.addEventListener('DOMMouseScroll', enAbleScroll, false); //Old Firefox

            document.removeEventListener('mousewheel', removeWheelEvent, false); //IE9, Chrome, Safari, Opera
            document.removeEventListener('wheel', removeWheelEvent, false); //Firefox
            document.removeEventListener('DOMMouseScroll', removeWheelEvent, false); //old Firefox
        } else {
            document.attachEvent('onmousewheel', enAbleScroll); //IE 6/7/8
            document.detachEvent('onmousewheel', removeWheelEvent); //IE 6/7/8
        }
    }

    function _disableScroll() {
        if (document.addEventListener) {
            document.addEventListener('mousewheel', removeWheelEvent, false); //IE9, Chrome, Safari, Oper
            document.addEventListener('wheel', removeWheelEvent, false); //Firefox
            document.addEventListener('DOMMouseScroll', removeWheelEvent, false); //Old Firefox

            document.removeEventListener('mousewheel', enAbleScroll, false); //IE9, Chrome, Safari, Oper
            document.removeEventListener('wheel', enAbleScroll, false); //Firefox
            document.removeEventListener('DOMMouseScroll', enAbleScroll, false); //old Firefox
        } else {
            document.attachEvent('onmousewheel', removeWheelEvent); //IE 6/7/8
            document.detachEvent('onmousewheel', enAbleScroll); //IE 6/7/8
        }
    }
